...
const oldCardRegex = /<div\s+key=\{`\$\{work\.title\}-\$\{activeFilter\}-\$\{idx\}`\}\s+className=\{`work-card group relative overflow-hidden rounded-sm cursor-pointer masonry-item`\}\s*>([\s\S]*?)<\/div>\s+<\/div>\s+<!-- CTA -->/g;

// To make it robust, let's just use string replace on the exact content between the map and the CTA.

const oldBlock = `        <div className="masonry-grid">
...
if (code.includes(oldBlock)) {
  ...
  console.log('Hover animation enhanced successfully!');
} else {
  console.log('Could not find the exact old block in Gallery.jsx');
}
